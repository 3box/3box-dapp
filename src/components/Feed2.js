...
class Feed extends Component {
  ...
  componentDidMount() {
    const { feedByAddress } = this.props;
    this.setState({
      renderFeed: feedByAddress.slice(0, 2),
    });
  }

  static getDerivedStateFromProps(nextProps) {
    const { feedByAddress } = nextProps;
    return {
      renderFeed: feedByAddress.slice(0, 2),
    };
  }
  ...
  loadActivity = () => {
    ...
    const { feedByAddress } = this.props;
    const nextFeedItems = feedByAddress.slice(page * 2, (page + 1) * 2);

    this.setState({
      page: page + 1,
      renderFeed: renderFeed.concat(nextFeedItems),
    });
  }

  render() {
    const { ifFetchingActivity, feedByAddress } = this.props;
    const { renderFeed } = this.state;
    console.log('renderFeed', renderFeed)
